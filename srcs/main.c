...
static void initData(t_data *data)
{
    ...
    data->name = ft_strdup("TUTORIAL");
	if (data->mlx_ptr == NULL)
		perror("ERROR_MLX");
	data->win_ptr = mlx_new_window(data->mlx_ptr,
			(nScreanWidth), (nScreanHeight), data->name);
    ...
    if (!data->img)
		perror("ERROR_MLX");
	data->img->mlx_img = mlx_new_image(data->mlx_ptr, 110, 30);
...
int main()
{
    ...
    while(1)
    {
        ...
        // mlx_key_hook(data->win_ptr, &hande_move, data);
